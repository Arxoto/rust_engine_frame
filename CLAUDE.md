...
- `design_patterns.rs` — **the central composition mechanism.** The design doc records why: `TickTimer` is self-sufficient while `StaticTimer` needs a `StaticTimeline`, and the code must not be duplicated for both. The chosen approach (**方案二**): traits declare their context via `DependCtx { type Ctx<'a>: Copy; }` as an associated type — `Ctx = ()` for self-sufficient types, `Ctx = &'a StaticTimeline` for timeline-dependent ones. `Union<T, U>` is the composition pair type used to build prefab proxies. `WithInto<Ctx>` (方案一, blanket-impl based) 已移入单元测试 mod 作为方案一的可运行参考,不属于公开接口;`Union` 构造统一为 `Union::new`(字段保持 `pub(super)`)。Read this file before adding any timer behavior.
...
- `attr_eff.rs` — `AttrEffect<S, Timer>` typed by `AttrEffectType` (BasicAdd/BasicPer/FinalPer/FinalMul) and `AttrModifier` with the formula `(basic_add + origin * (1 + basic_per)) * (1 + final_per) * final_mul`. 基线值同时用作默认初始值与增益/减益判断基线：加法类（BasicAdd/BasicPer/FinalPer）为 0，乘法类（FinalMul）为 1。Implements `Upsert`.
...
- `upsert_container.rs` — `UpsertContainer<E: Upsert>`: `Vec<Option<E>>` effect list with **upsert-by-id** (id = `(effect_name, from_name)`), **修改即置脏** dirty flag (any mutating access dirties — see CONTEXT.md), hole-counting compaction (`try_clean_hole`, threshold 25% or >50 holes), and default merge strategy `upsert_replace`. `UpsertContainerCleaner` self-holds the default period (`time_type::DEFAULT_REFRESH_PERIOD`, 5s) with single-entry `clean_holes`. Docs note: array storage beats `FxHashMap` up to ~20–50 elements.
- `attr_systems.rs` — the ECS-style **System** layer, split into generic functions (not one monolithic `process_tick`): `clean_expired_element` (generic via `HasTimer`), `try_refresh_dirty_attr`, `try_clean_hole` (薄委托于 `UpsertContainerCleaner::clean_holes`), `try_reset_timeline`. They are free generic functions meant to be composed per-entity by the caller — the module's test (`example_process_tick`) shows an OO-style composition. (combats does not yet invoke them.)
...
- `animations.rs` — 已并入 `motions.rs`(`pub mod animations;`):当前仅持有「动画播放器结束信号自动衔接下一段动画」的过渡设计模块文档,无代码。
