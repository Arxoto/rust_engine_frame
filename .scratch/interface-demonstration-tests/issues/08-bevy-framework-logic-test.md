...
已实现（commit 45d3402）：
- `tests/bevy_tests.rs` 新增属性效果刷新链测试:本地 newtype 把 `UpsertContainer<AttrEffect<String, TickTimer>>` 包装为 Bevy Resource(孤儿规则适配),`per_entity_tick_system` 内调用 `clean_expired_element` + `try_refresh_dirty_attr`,挂 `Update` Schedule。
- 推进 3 帧断言:效果生效(100+20+5=125)→ 有限时长效果过期回落(105)→ 无变更保持(105)。
- 在 `--no-default-features --features bevyproj,time_type_f64` 下通过,验证同一套 `base_lib` 逻辑与 Godot 侧共用、引擎无关。
