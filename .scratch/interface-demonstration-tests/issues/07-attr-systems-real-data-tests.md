...
已实现（commit 45d3402）：
- `attr_systems.rs` 新增 `real_data_per_entity_tick`（正常帧组合:clean → refresh_dirty → clean_hole）与 `real_data_timeline_reset`(大 tick 越过一年门槛 → `try_reset_timeline`),用带效果的容器 + 时间线 + 过期效果断言刷新、清理、时间线重置全链路。
- 保留 `example_process_tick` 为 OOS 样板。
- `try_reset_timeline` 首次获得覆盖(含 `fix_timeline_diff` 对依赖计时器的影响)。
