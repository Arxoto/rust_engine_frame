...
本项目意图实现一个“无游戏引擎依赖”的游戏通用底层开发框架，游戏业务逻辑通过上层调用 `pub fn` 来驱动。

因此部分方法可能没有在本项目内被直接调用，但是会有文档测试或单元测试演示如何使用。
...
- 若在 Bevy 引擎中使用，则属于原生语言开发，完全没有额外的性能损耗（但要注意数据结构符合 ECS 用法）
...
- HashMap/HashSet 的性能瓶颈
  ...
  - 另一方面，预分配容量也是一个优化点 `Vec::with_capacity(capacity.next_power_of_two())` ，避免重新分配和重新哈希
    - 注意出于内存对齐的目的，初始化时 capacity 尽量做 2 的幂次优化（因为后续扩容也是翻倍扩容的），如果是能够提前确定元素个数的，需要做注释和标记说明
    - 使用命令进行检查 `grep -r 'with_capacity' . | grep -v 'next_power_of_two'`
