    ...
    for OnFloorBehaviour<S>
{
    ...
    fn on_enter(&mut self, p: &PhyParam<S>) {
        if p.character_landing {
            self.landing_timer.restart();
        } else {
            self.landing_timer.pause();
        }
        self.ready_jump_timer.pause();
    }

    fn tick_frame(&mut self, p: &FrameParam<S>) -> FrameEff<S> {
        // 着陆缓冲动画（仅视觉效果）
        self.landing_timer.tick(p.delta);
        if self.landing_timer.is_timing() {
            return FrameEff::from(self.landing_anim.clone());
        }

        if self.ready_jump_timer.is_timing() {
            // 若资源充足可制作跑步起跳动画
    ...
    fn process_physics(&mut self, (p, data): &mut (&mut PhyParam<S>, &MotionData)) -> PhyEff {
        ...
        self.ready_jump_timer.tick(p.delta);
        if p.instructions.jump_once.op_active() {
            ...
            if !self.ready_jump_timer.is_timing() {
                self.ready_jump_timer.restart();
            }
        }
        if self.ready_jump_timer.is_finished() {
