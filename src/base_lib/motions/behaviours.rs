...
//! - 物理效果（根据需求每帧或首次触发）
//!   - 速度（瞬间改变速度，部分动作生成此效果）
//!   - 加速度和最大值（逐渐改变效果，基础行为可以生成这个效果）
//!   - 无效果（应用依赖的行为的效果）
//! - 视觉效果（一般仅首次触发）
//!   - 是否允许图像翻转（大部分动作都不应该翻转）
//!   - 动画（大部分都应该生成此效果，按照优先级覆盖）
//!     - 可拓展，比如上下半身动画分离以实现高级功能
//!   - 特效（特效不会被覆盖，全部生效）
//!
//! ## 动作行为的集成测试点
//!
...
///
/// 根据需求待补充
pub mod under_water_behaviour {}

/// 攀爬一般行为
///
/// 播放攀爬过渡动画，播放完成后自动播放攀爬动画，过渡动画参考时长 0.2s
pub mod climb_wall_behaviour {}
