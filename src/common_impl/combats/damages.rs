...
use crate::{
    ...
    common_impl::combats::{
        ...
        combat_units::{
            Health, Magicka, ShieldArcane, ShieldDefence, ShieldSubstitute, SurvivalAttrLayer,
        },
    },
};
        ...
        for dmg_eff in survival_eff_buffer.0.drain(..) {
            ...
            let (base_bounded, base_bound) = match target_type.stop_at() {
                ...
                SurvivalAttrLayer::ShieldDefence => {
                    (&target_shield_def.0, &target_shield_def_upper.0)
                }
                SurvivalAttrLayer::ShieldArcane => {
                    (&target_shield_arc.0, &target_shield_arc_upper.0)
                }
            };
